...
RedisDriver.prototype.readFile = function(filename, encoding, callback) {
    filename = pathToKey(filename);
    var redisCli = this.redisCli;
    ...
    redisCli.hmget(filename, '_type', 'ino', function(err, info) {
        ...
        var type = info[0], ino = info[1];
        if (type == 'file') {
            redisCli.get(new Buffer('///file' + ino), function(err, buf) {
                if (err) {
                    return callback && callback(err);
                }
                if (encoding) {
                ...
                return callback && callback(null, buf);
            });
        } else if (type == 'directory') {
            return callback && callback(filename + ' is a directory');
        } else {
            return callback && callback(filename + ' does not exist');
        }
    });
}

RedisDriver.prototype.stat = function(path, callback) {
    path = pathToKey(path);
    var redisCli = this.redisCli;
    redisCli.exists(path, function(err, exists) {
        if (err) {
            return callback && callback(err);
        }
        if (!exists) {
            return callback && callback(path + ' does not exist');
        }

        redisCli.hgetall(path, function(err, info) {
            if (err) {
                return callback && callback(err);
            }

            return callback && callback(null, Stats(info));
        });
    });
}
